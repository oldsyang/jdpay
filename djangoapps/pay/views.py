...
def index(request):
    pc_url = create_order(request)
    settings.PAY_URL = settings.MOBILE_PAY_URL
    mobile__url = create_order(request)
    return render(request, "index.html", {'mobile_url': mobile__url.format('h5手机支付'), 'pc_url': pc_url.format('电脑支付')})
